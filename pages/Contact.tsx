...
import { CONTACT_INFO, HQ_ADDRESS, POSTAL_ADDRESS, BRAND_NAME } from '../constants';
...
const Contact: React.FC = () => {
  ...
  return (
    <div className="animate-fadeIn">
      ...
      {/* ── HEADER ── */}
      <section className="bg-surface-50 py-24 border-b border-border-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="font-mono text-[11px] tracking-[0.2em] text-ink-700/50 uppercase mb-4">[CONTACT US]</p>
          <h1 className="text-5xl md:text-6xl font-heading font-bold text-ink-900 mb-4">Let's Build the<br/>Future Together</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            ...
            <div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                <div className="bg-surface-50 p-6 rounded-card border border-border-200">
                  ...
                  <p className="font-mono text-[10px] text-ink-700/50">Postal: {POSTAL_ADDRESS}</p>
                </div>
                <div className="bg-surface-50 p-6 rounded-card border border-border-200">
                  <div className="font-mono text-[10px] tracking-widest uppercase text-brand-600 mb-3">Reach Out</div>
                  <p className="text-sm font-medium text-ink-900 mb-1">E: {CONTACT_INFO.email}</p>
              ...
              {/* Map placeholder */}
              <div className="rounded-card overflow-hidden border border-border-200 bg-surface-50 relative h-[360px] flex items-center justify-center">
                <div className="text-center max-w-xs p-8">
                  <div className="w-14 h-14 bg-white rounded-card border border-border-200 flex items-center justify-center mx-auto mb-4">
                    <svg className="w-6 h-6 text-ink-900/30" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/>
                    </svg>
                  </div>
                  <p className="font-heading font-bold text-ink-900 mb-1">Hazeldean Office Park</p>
            ...
            {/* Contact Form */}
            <div className="bg-white rounded-card border border-border-200 shadow-xl p-8 md:p-10">
              {submitted ? (
                <div className="text-center py-12 animate-fadeIn">
                  <div className="w-20 h-20 bg-ink-900 text-white rounded-full flex items-center justify-center mx-auto mb-8">
                    <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path>
                    </svg>
                  </div>
                  <p className="font-mono text-[10px] tracking-widest uppercase text-ink-700/50 mb-3">[SUCCESS]</p>
                  <h3 className="text-2xl font-heading font-bold text-ink-900 mb-4">Message Sent!</h3>
                  <p className="text-ink-700 mb-8 leading-relaxed">Thanks for reaching out. We will reply shortly. Want to speed this up?</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="mb-6">
                    ...
                    <h3 className="text-2xl font-heading font-bold text-ink-900">Get in Touch</h3>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <input
                        required
                        className="w-full px-4 py-3 border border-border-200 rounded-xl bg-surface-50 focus:ring-2 focus:ring-brand-600 outline-none text-sm transition-all"
                        value={formData.name}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea
                      required
                      rows={4}
                      className="w-full px-4 py-3 border border-border-200 rounded-xl bg-surface-50 focus:ring-2 focus:ring-brand-600 outline-none resize-none text-sm transition-all"
      ...
      {/* ── FAQ ── Clean accordion with + icons */}
      <section className="py-24 bg-surface-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <p className="font-mono text-[11px] tracking-[0.2em] text-ink-700/50 uppercase mb-3">/ FAQ</p>
            <h2 className="text-4xl font-heading font-bold text-ink-900">Everything You Need to Know—Upfront</h2>
            <p className="text-ink-700 mt-3 max-w-xl mx-auto">
          ...
          <div className="space-y-3">
            {faqs.map((faq, idx) => (
              <div key={idx} className="bg-white border border-border-200 rounded-card overflow-hidden">
                <button
                  className="w-full px-6 py-5 flex justify-between items-center text-left hover:bg-surface-50 transition-colors"
                  onClick={() => setOpenFaq(openFaq === idx ? null : idx)}
                >
                  <span className="font-bold text-ink-900 font-heading pr-4">{faq.q}</span>
                  <div className={`w-8 h-8 rounded-full border border-border-200 flex items-center justify-center shrink-0 transition-all ${openFaq === idx ? 'bg-ink-900 border-ink-900' : 'bg-white'}`}>
                    <span className={`text-lg font-bold leading-none transition-colors ${openFaq === idx ? 'text-white' : 'text-ink-900'}`}>
                      {openFaq === idx ? '−' : '+'}
                    </span>
                  </div>
                </button>
                {openFaq === idx && (
                  <div className="px-6 pb-6 text-ink-700 text-sm leading-relaxed border-t border-border-200 pt-4 animate-fadeIn">
                    {faq.a}
                  </div>
                )}
              </div>
